...
int species = GetBMIService.GetUserSpeciesOrGender<ESpecies>("物種");  //判斷物種格式是否正確
int gender = GetBMIService.GetUserSpeciesOrGender<EGender>("性別");  //判斷性別格式是否正確
double height = GetBMIService.GetUserWeightOrHeight("請輸入您的身高(cm):"); //判斷身高格式是否為正數數字
double weight = GetBMIService.GetUserWeightOrHeight("請輸入您的體重(kg):"); //判斷體重格式是否為正數數字
